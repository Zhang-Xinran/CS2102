...
drop function if exists update_room, remove_session, add_session, view_manager_report, view_summary_report;
...
drop function if exists popolar_courses;
drop function if exists insert_offering_check_admin_func, insert_offering_check_date_func,
insert_session_check_eid_func, insert_session_check_room_func, limit_active_package_per_person,
limit_session_per_person, limit_session_quota, limit_session_time_in_redeems, limit_session_time_in_registers,
popular_courses, update_offerings_date, update_offerings_seating_capacity cascade;
...
drop function if exists get_remaining_seats_offering();
drop function if exists  get_number_of_registration();
drop procedure if exists add_customer(text,text,text,text,text,date,integer);
drop procedure if exists update_credit_card(integer,text,date,integer);
drop function if exists insert_session_check_eid_func();
drop trigger if exists insert_session_check_eid_trigger on sessions;
drop function if exists insert_session_check_room_func();
drop trigger if exists insert_session_check_room_trigger on sessions;
drop function if exists insert_offering_check_date_func();
drop trigger if exists insert_offering_check_date_trigger on offerings;
drop function if exists insert_offering_check_admin_func();
drop trigger if exists insert_offering_check_admin_trigger on offerings;
drop type session_type cascade;
...
BEGIN
	...
	THEN work_day:= null;
	ELSEIF ( (select extract(month from (select depart_date from employees where eid=employee_id))) < (select extract(month from current_date))
        and (select extract(year from (select depart_date from employees where eid=employee_id)))<=(select extract(year from current_date)) )
  THEN work_day:=0;
  ELSE
		first_work_day:=(
			select case
      ...
      or ( (select extract(month from depart_date))>(select extract(month from current_date))
        and (select extract(year from depart_date))>=(select extract(year from current_date)) )
...
index_no integer;
duration integer := (select duration from courses where course_id = course_id1);


BEGIN
        IF (current_date > registration_deadline) THEN
               RAISE NOTICE 'Registration deadline has passed for the offering. Fail to add a new offering.';
               RETURN;
           END IF;
        ...
        foreach s in array session_info loop
           ...
           IF (SELECT COUNT(*) FROM find_instructors(course_id1,session_date,start_time)) = 0 THEN
               ...
               RETURN;
           ELSIF (registration_deadline > session_date::date - 10*interval '1' day) THEN
               RAISE NOTICE 'Date early than/too close to register deadline';
               RETURN;
           ELSIF (room_id NOT IN  (select * from find_rooms (session_date, start_time, duration))) THEN
               RAISE NOTICE 'Room not available';
               RETURN;
           ELSIF (start_time+duration > 12 OR start_time+duration>18) THEN
               RAISE NOTICE 'Session not able to conduct at this time';
               RETURN;
           ELSIF(session_date < registration_deadline) THEN
               RAISE NOTICE 'Session date is later than the registration deadline. Logical error. Fail to add session and offering.';
               RETURN;
           END IF;
...
returns integer as $$
declare
	c cursor for (select * from Redeems R where R.buy_date = buy_d and R.cust_id = cid and R.card_number = card and R.package_id = pid);
	r record;
begin
	res := 0;
	open c;
	loop
		fetch c into r;
		exit when not found;
		if ((select session_date from Sessions S where S.sid = r.sid and S.offering_id = r.offering_id) - (select current_date) > 7) then
			res := 1;
		end if;
	end loop;
  close c;
...
begin
	if (paymentMethod = 1) then
		...
		if (select available_package is null) then
...
BEGIN
     ...
     ELSE
     UPDATE Sessions SET rid = new_rid
...
BEGIN
  ...
  and (select extract(year from payment_date))=(select extract(year from current_date));
END;
$$ LANGUAGE plpgsql;
...
BEGIN
   ...
   package_sum as (
   select A.year,A.month,sum(package_price) as total_package_sold,0.00 as total_registration_fee,0.00 as total_salary_paid,0.00 as total_refunded_registration_fee,0.0::bigint as total_redemption
   from package A
   ...
   card_sum as (
   select A.year,A.month,0.00 as total_package_sold,sum(offering_price) as total_registration_fee,0.00 as total_salary_paid,0.00 as total_refunded_registration_fee,0.0::bigint as total_redemption
   from card A
   group by A.year,A.month
   ),
   salary as (
   ...
   salary_sum as (
   select A.year,A.month,0.00 as total_package_sold,0.00 as total_registration_fee,sum(salary_amount) as total_salary_paid,0.00 as total_refunded_registration_fee,0.0::bigint as total_redemption
   from salary A
   group by A.year,A.month
   ),
   cancellation as (
   ...
   cancellation_sum as (
   select A.year,A.month,0.00 as total_package_sold,0.00 as total_registration_fee,0.00 as total_salary_paid,sum(refund_amount) as total_refunded_registration_fee,0.0::bigint as total_redemption
   from cancellation A
   group by A.year,A.month
   ),
   redemption as (
   ...
   redemption_sum as (
   select A.year,A.month,0.00 as total_package_sold,0.00 as total_registration_fee,0.00 as total_salary_paid,0.00 as total_refunded_registration_fee,count(*) as total_redemption
   from redemption A
   group by A.year,A.month
   ),
   combined as (
   select * from package_sum
   union
   select * from card_sum
   union
   select * from salary_sum
   union
   select * from cancellation_sum
   union
   select * from redemption_sum
   )

   select C.year,C.month,sum(C.total_package_sold),sum(C.total_registration_fee),sum(C.total_salary_paid),sum(C.total_refunded_registration_fee),sum(C.total_redemption)
   from combined C
...
BEGIN
     IF (TG_OP = 'INSERT' or TG_OP = 'UPDATE') THEN
         UPDATE Offerings SET start_date = (
...
BEGIN
   IF NEW.eid in (
       select employee_id from find_instructors((select course_id from offerings where offering_id = NEW.offering_id), NEW.session_date, NEW.start_time)
       ) THEN
       RETURN NEW;
   ELSE
...
begin
    ...
    raise notice 'Exceeds seating capacity.';
		return null;
	end if;
end;
$$ language plpgsql;
...
create or replace function limit_session_time_in_registers()
returns trigger as $$
declare
	registration_deadline date;
	operation_date date;
begin
	...
	if (registration_deadline >= (new.registration_date)) then
		return new;
	else
...
create or replace function limit_session_time_in_redeems()
returns trigger as $$
declare
	registration_deadline date;
	operation_date date;
begin
	registration_deadline := (select O.registration_deadline from Offerings O where O.offering_id = new.offering_id);
	if (registration_deadline >= new.redemption_date) then
		return new;
	else
		raise notice 'The registration has ended.';
		return null;
...
before insert or update on Registers
for each row execute function limit_session_time_in_registers();

create trigger limit_session_time_in_redeems
before insert or update on Redeems
for each row execute function limit_session_time_in_redeems();
